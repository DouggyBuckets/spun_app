...
router.get("/albums/:spotifyId", requireAuth, async (req, res) => {
    ...
    const result = await db.query(
        `SELECT id FROM likes WHERE user_id = $1 AND entity_type = 'album' AND entity_id = $2`,
        [req.user!.id, albumId]
    );
...
router.post("/songs/:spotifyId", requireAuth, async (req, res) => {
    const { albumSpotifyId } = likeSongSchema.parse(req.body);
...
router.delete("/albums/:spotifyId", requireAuth, async (req, res) => {
    ...
    await db.query(
        `DELETE FROM likes WHERE user_id = $1 AND entity_type = 'album' AND entity_id = $2`,
        [req.user!.id, albumId]
    );
...
router.delete("/songs/:spotifyId", requireAuth, async (req, res) => {
    ...
    await db.query(
        `DELETE FROM likes WHERE user_id = $1 AND entity_type = 'song' AND entity_id = $2`,
        [req.user!.id, songId]
    );
    res.json({ message: "Unliked successfully" });
});
